...
const DoctorDashboard = ({ account, consentContract, auditLogContract, accessContract, medicalRecordsContract, walletMapperContract }) => {
    ...
    return (
        <div className="dashboard animate-fade-in" style={{ padding: '2rem' }}>
            ...
            <div className="dashboard-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                ...
                <div className="surface-card" style={{ padding: '2rem' }}>
                    ...
                    <ul style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: '1.6' }}>
                        <li style={{ marginBottom: '0.5rem' }}>✔ Ensure patient has granted active digital consent.</li>
                        <li style={{ marginBottom: '0.5rem' }}>✔ All data access is logged to the immutable audit trail.</li>
                        <li style={{ marginBottom: '0.5rem' }}>✔ Respect "Right to Erasure" requests from patients.</li>
                    </ul>
                </div>
            </div>

            {/* Prescription Upload Panel */}
            <div className="dashboard-section surface-card" style={{ marginTop: '2rem', padding: '2rem', borderTop: '4px solid var(--primary-color)' }}>
                ...
                <form onSubmit={handleUploadPrescription} style={{ display: 'grid', gridTemplateColumns: 'minmax(200px, 1fr) minmax(200px, 1fr)', gap: '1rem' }}>
                    <div className="form-group">
                        <label>Patient Wallet Address</label>
                        <input type="text" placeholder="0x..." value={rxPatientWallet} onChange={(e) => setRxPatientWallet(e.target.value)} required />
                    </div>
                    <div className="form-group">
                        <label>Patient Name (Alias)</label>
                        <input type="text" placeholder="e.g. John Doe" value={rxPatientName} onChange={(e) => setRxPatientName(e.target.value)} required />
                    </div>
                    <div className="form-group">
                        <label>Medication</label>
                        <input type="text" placeholder="e.g. Amoxicillin 500mg" value={rxMedicine} onChange={(e) => setRxMedicine(e.target.value)} required />
                    </div>
                    <div className="form-group" style={{ display: 'flex', gap: '1rem' }}>
                        <div style={{ flex: 1 }}>
                            <label>Dosage</label>
                            <input type="text" placeholder="1 tablet twice daily" value={rxDosage} onChange={(e) => setRxDosage(e.target.value)} required />
                        </div>
                        <div style={{ flex: 1 }}>
                            <label>Duration</label>
                            <input type="text" placeholder="7 days" value={rxDuration} onChange={(e) => setRxDuration(e.target.value)} required />
                        </div>
                    </div>
            ...
            <div className="dashboard-section surface-card" style={{ marginTop: '2rem', padding: '2rem', borderTop: '4px solid var(--accent-color)' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }} className="form-group">
                    <input
                        type="text"
                    ...
                    />
                    <button className="primary-btn" style={{ padding: '0.85rem 1.5rem', height: '100%' }} onClick={handleDecryptRecord} disabled={isDecrypting}>
            ...
            <div className="dashboard-section surface-card" style={{ marginTop: '2rem', padding: '2rem' }}>
